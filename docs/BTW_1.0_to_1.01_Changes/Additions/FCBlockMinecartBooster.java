...
public class FCBlockMinecartBooster extends Block {
	public FCBlockMinecartBooster(int iBlockID, int iTextureIndex) {
		super(iBlockID, iTextureIndex, Material.iron);
	}

	public int idDropped(int i, Random random) {
		return mod_FCBetterThanWolves.fcMinecartBoosterIdle.blockID;
	}

	public int getBlockTextureFromSide(int side) {
		return side == 1 ? (this.blockID == mod_FCBetterThanWolves.fcMinecartBoosterActive.blockID ? 104 : 104) : this.blockIndexInTexture;
	}

	public boolean IsBoosterOn(World world, int i, int j, int k) {
		return world.getBlockId(i, j, k) == mod_FCBetterThanWolves.fcMinecartBoosterActive.blockID;
	}
	...
	private void BoosterTurnOn(World world, int i, int j, int k) {
		...
		world.setBlockWithNotify(i, j, k, mod_FCBetterThanWolves.fcMinecartBoosterActive.blockID);
	}

	private void BoosterTurnOff(World world, int i, int j, int k) {
		world.playSoundEffect((double)((float)i + 0.5F), (double)((float)j + 0.5F), (double)((float)k + 0.5F), "mob.ghast.moan", 0.5F, 2.6F + (world.rand.nextFloat() - world.rand.nextFloat()) * 0.8F);
		world.setBlockWithNotify(i, j, k, mod_FCBetterThanWolves.fcMinecartBoosterIdle.blockID);
	}
