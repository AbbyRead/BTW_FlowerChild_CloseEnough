...
public class ModTexture extends TextureFX {
	...
	public ModTexture(int slot, int dst, BufferedImage source) {
		super(slot);
		this.tileImage = dst;
		int width = source.getWidth();
		int height = source.getHeight();
		int requiredsize = (int)Math.sqrt((double)(this.imageData.length / 4));
		this.pixels = new int[requiredsize * requiredsize];
		if(width == height && width == requiredsize) {
			...
			gfx.drawImage(source, 0, 0, requiredsize, requiredsize, 0, 0, width, height, (ImageObserver)null);
			img.getRGB(0, 0, requiredsize, requiredsize, this.pixels, 0, requiredsize);
			gfx.dispose();
		}
